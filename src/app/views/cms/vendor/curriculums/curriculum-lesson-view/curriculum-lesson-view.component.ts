...
@Component({
    selector: 'app-cms-vendor-curriculum-lesson-view',
...
export class CurriculumLessonViewComponent implements OnInit, OnDestroy {
    ...
    updateLesson(index) {
        const data = {
            ...
            previewPerc: JSON.parse(this.lessonForm.get('items')['controls'][index].controls['previewPerc'].value.toString())
        };
                ...
                .subscribe(
                    (resp: any) => {
                        ...
                        if (resp.status === 200) {
                            ...
                            document.getElementById('videoArea_' + _this.selectedVideoNo).setAttribute('style', 'display: block');
