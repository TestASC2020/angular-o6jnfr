...
@Component({
  selector: 'app-cms-creator-curriculum-exercise-view',
...
export class CurriculumExerciseViewComponent implements OnInit, OnDestroy {
  ...
  saveExercise() {
    ...
    this.service.updateExercise(data).subscribe(res => {
      const index = this.exerciseList.map(item => item.signature).indexOf(this.selectedExercise.toString());
      this.exerciseList[index].name = data.name;
      this.resetFromSelected();
    });
  }
  ...
  viewSolution() {
    const index = this.exerciseList.map(item => item.signature).indexOf(this.selectedExercise);
    const sendExercise = {
    ...
    this.router.navigate(['cms/creator/curriculums/curriculum-view/curriculum-unit-list-edit/curriculum-exercise-view/edit-solution']);
  }
