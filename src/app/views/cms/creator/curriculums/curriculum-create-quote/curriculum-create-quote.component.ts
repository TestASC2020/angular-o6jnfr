...
@Component({
  selector: 'app-cms-creator-curriculum-create-quote',
...
export class CurriculumCreateQuoteComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    this.quoteFormGroup = new FormGroup({
      ...
      wishMinPrice: new FormControl(0, [Validators.required]),
      wishMaxPrice: new FormControl(0, [Validators.required]),
      ...
      this.service.loadCv({cvSig: this.cv.signature}).subscribe(result => {
        ...
        this.service.loadCvQuoteRqt({cvSig: this.cv.signature}).subscribe(quoteLoad => {
          if (quoteLoad['data']) {
            ...
            this.service.addCvQuoteRqt({cvSig: this.cv.signature}).subscribe(quoteLoad2 => {
              this.rqtQuote = quoteLoad2['data'];
              this.initFromQuote();
            });
          }
        });
