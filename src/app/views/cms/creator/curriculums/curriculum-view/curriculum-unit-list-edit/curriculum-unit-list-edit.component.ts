...
@Component({
    selector: 'app-cms-creator-curriculum-unit-list-edit',
...
export class CurriculumUnitListEditComponent implements OnInit, OnDestroy {
    ...
    updateReferencesUnitPrerequisites(deletedUnitIndex: number, deletedUnitNo: number) {
      if (deletedUnitNo) {
        for (let i = deletedUnitIndex; i < this.cv.units.length; i++) {
    ...
    openUnitInfoModal() {
        ...
        for (let i = 0; i < this.cv.units.length; i++) {
            ...
            let check = false;
            if (this.cv.units[i]['prerequisites'] && this.cv.units[i]['prerequisites'].length > 0) {
                check = !(this.cv.units[i]['prerequisites'].indexOf(this.cv.units[i]['no']) < 0);
                prerequisites.addControl((this.cv.units[i]['no']).toString(), new FormControl(check));
            } else {
                prerequisites.addControl((this.cv.units[i]['no']).toString(), new FormControl(false));
            }
    ...
    editLesson(index, lesson) {
        ...
        this.router.navigate(['cms/creator/curriculums/curriculum-view/curriculum-unit-list-edit/lesson-view']);
    }
    ...
    editExercise(index, exercise) {
        ...
        this.router.navigate(['cms/creator/curriculums/curriculum-view/curriculum-unit-list-edit/curriculum-exercise-view']);
    }
    ...
    editQuestion(index, question) {
        ...
        this.router.navigate(['cms/creator/curriculums/curriculum-view/curriculum-unit-list-edit/curriculum-question-view']);
    }
