...
@Component({
    selector: 'app-profile-org-group',
    templateUrl: './profile-org-group.component.html',
    styleUrls: ['./profile-org-group.component.scss']
})
export class ProfileOrgGroupComponent implements OnInit {
    ...
    ngOnInit() {
        this.service.loadUserProfile().subscribe(response => {
            this.service.loadCountriesList().subscribe(res => {
                ...
                this.profileForm.get('about').setValue((this.profiles && this.profiles['about']) ? this.profiles['about'] : '');
                this.profileForm.get('phone').setValue((this.profiles && this.profiles['phone']) ? this.profiles['phone'] : '');
                this.profileForm.get('employer').setValue((this.profiles && this.profiles['employer']) ? this.profiles['employer'] : '');
                if (!this.profileForm.get('country')) {
                  ...
                  if (this.profiles) {
                    if (this.countriesList.map(it => it.name).indexOf(this.profiles['country']) < 0) {
                      ...
                      this.profileForm.get('country').setValue(
                        this.countriesList[this.countriesList.map(it => it.name).indexOf(this.profiles['country'])].code);
                    }
                  }
                }
    ...
    updateUser() {
        const data = {
            ...
            address: {
                country: (this.countriesList && this.countriesList.length > 0) ? this.countriesList[this.countriesList.map(it => it.code).indexOf(this.profileForm.get('country').value)].name : '',
                region: (this.regionList && this.regionList.length > 0) ? this.regionList[this.regionList.map(it => it.region).indexOf(this.profileForm.get('region').value)].region : '',
                city: (this.citiesList && this.citiesList.length > 0) ? this.citiesList[this.citiesList.map(it => it.code).indexOf(this.profileForm.get('city').value)].name : '',
    ...
    updateExperienceControl(index) {
        ...
        this.toastrService.clear();
        const notifNeigbourId = '#notif-exp-neigbour';
        this.service.updateUserExperience(data).subscribe(
            resp => {
                const control = (<FormArray>this.experiencesForm.get('experiences')).controls[index];
